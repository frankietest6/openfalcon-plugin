...
// then filters to only those that are actually `php …openfalcon_listener.php`
...
foreach ($lines as $line) {
    ...
    $pid = $parts[0];
    $cmd = $parts[1];
    // Only count processes whose command line is `php … openfalcon_listener.php`
    if (preg_match('#(^|/)php\s+.*openfalcon_listener\.php#', $cmd)) {
        $pids[] = $pid;
    }
}
